...
class WhatsAppWeb(WebSocket):
    ...
    def handleMessage(self):
        try:
            eprint("SELF DATA", self.data);
            tag = self.data.split(",", 1)[0];
            obj = json.loads(self.data[len(tag) + 1:]);

            eprint(obj);
            if "from" not in obj or obj["from"] != "api2backend" or "type" not in obj or not (("command" in obj and obj[
                "command"] == "backend-connectWhatsApp") or "whatsapp_instance_id" in obj):
                ...
                if obj["command"] == "backend-connectWhatsApp":
                    ...
                    onOpenCallback = {
                        "func": lambda cbSelf: self.sendJSON(
                            mergeDicts({"type": "resource_connected", "resource": "whatsapp"}, getAttr(cbSelf, "args")),
                            getAttr(cbSelf, "tag")),
                        "tag": tag,
                        "args": {"resource_instance_id": clientInstanceId}
                    };
                    onMessageCallback = {
                        "func": lambda obj, cbSelf, moreArgs=None: self.sendJSON(mergeDicts(mergeDicts(
                            {"type": "whatsapp_message_received", "message": obj, "timestamp": getTimestampMs()},
                            getAttr(cbSelf, "args")), moreArgs), getAttr(cbSelf, "tag")),
                        "args": {"resource_instance_id": clientInstanceId}
                    };
                    onCloseCallback = {
                        "func": lambda cbSelf: self.sendJSON(
                            mergeDicts({"type": "resource_gone", "resource": "whatsapp"}, getAttr(cbSelf, "args")),
                            getAttr(cbSelf, "tag")),
                        "args": {"resource_instance_id": clientInstanceId}
                    };
                    self.clientInstances[clientInstanceId] = WhatsAppWebClient(onOpenCallback, onMessageCallback,
                                                                               onCloseCallback);
                else:
                    currWhatsAppInstance = self.clientInstances[obj["whatsapp_instance_id"]];
                    callback = {
                        "func": lambda obj, cbSelf: self.sendJSON(mergeDicts(obj, getAttr(cbSelf, "args")),
                                                                  getAttr(cbSelf, "tag")),
                        "tag": tag,
                    ...
                    if cmd == "backend-generateQRCode":
                        ...
                        num = obj["number"];
                        text = obj["message"];
                        currWhatsAppInstance.sendTextMessage(num, text);
                    elif cmd == "backend-checkNumber":
                        num = obj["number"];
                        id = obj["id"];
                        currWhatsAppInstance.checkNumber(num, id, callback);
        except:
