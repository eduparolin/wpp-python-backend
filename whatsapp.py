...
def WhatsAppEncrypt(encKey, macKey, plaintext):
    eprint("encKey            >>>>>>>>>>: ", base64.b64encode(encKey));
    eprint("macKey                 >>>>>>>>: ", base64.b64encode(macKey));
    eprint("playText          >>>>>>>>>: ", plaintext);
...
class WhatsAppWebClient:
    ...
    def generateQRCode(self, callback=None):
        ...
        message = messageTag + ',["admin","init",[0,3,416],["Chromium at ' + datetime.datetime.now().isoformat() + '","Chromium"],"' + \
                  self.loginInfo["clientId"] + '",true]';
        self.activeWs.send(message);

    def restoreSession(self, callback=None):
        messageTag = str(getTimestamp())
        message = messageTag + ',["admin","init",[0,3,416],["Chromium at ' + datetime.now().isoformat() + '","Chromium"],"' + \
                  self.loginInfo["clientId"] + '",true]'
        self.activeWs.send(message)
    ...
    def checkNumber(self, number, id, callback):
        messageTag = str(id);
        message = messageTag + ',,["query", "exist", "' + number + '"]'

        self.activeWs.send(message)
        self.messageQueue[messageTag] = {"desc": "_checkNumber", "callback": callback};

        callback["func"]({"type": "exist", "data": messageTag}, callback);
